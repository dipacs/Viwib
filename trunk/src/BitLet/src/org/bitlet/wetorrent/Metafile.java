...
public class Metafile extends Bencode {
    ...
    private List<Map> files = new LinkedList<Map>();
    private String name;
    ...
    public Metafile(InputStream is) throws IOException, NoSuchAlgorithmException {
        ...
        if (files != null) {
            ...
            for (Object fileObj : files) {
                ...
                Long len = (Long) file.get(ByteBuffer.wrap("length".getBytes()));
                LinkedList pathList = (LinkedList) file.get(ByteBuffer.wrap("path".getBytes()));
                String path = "";
                for (Object o : pathList) {
                    if (!path.equals("")) {
                        path += File.separator;
                    }
                    path += new String(((ByteBuffer) o).array());
                }
                this.files.add(file);
                this.lengths.add(len);
                this.offsets.add(actOffset);
                this.paths.add(path);
